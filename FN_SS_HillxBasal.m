...
% FN_SS_HillxBasal : Finding steady state solution for the simple Hill
%                     model given a set of biophysical paramers and
%                     inducer (hormone) concentration.
%
%   Xs = FN_SS_HillxBasal(X,H,p)
...
function Ye = FN_SS_HillxBasal(X,H,p)
    OHFn = @(x,n,k,a) [a+((1-a)*(x.^n)./((x.^n)+(k^n)))];
    Ye = zeros(length(H),length(X));
    for h = 1:length(H)
        for i = 1:length(X)
            Ye(h,i) = ((p.m * OHFn(H(h)*X(i),p.n,p.K,p.a)) ...
                     + (p.b * X(i)))/p.g;
        end
    end
    clear h i
